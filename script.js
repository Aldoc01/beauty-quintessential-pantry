...
const Lightbox = {
    ...
    init() {
        ...
        this.lightboxSize = document.getElementById('lightboxSize');
        ...
        // Build images array from products
        this.images = CONFIG.PRODUCTS;
    ...
    open(productId) {
        const index = this.images.findIndex(p => p.id === productId);
        if (index === -1) return;
    ...
    updateDisplay() {
        const product = this.images[this.currentIndex];
        this.lightboxImage.src = product.image;
        this.lightboxImage.alt = `${product.name} - ${product.size} size`;
        this.lightboxTitle.textContent = product.name;
        this.lightboxSize.textContent = `Size: ${product.size}`;
        this.lightboxPrice.textContent = `₦${product.price.toLocaleString()}`;
...
function setupEventListeners() {
    ...
    DOM.productsGrid.addEventListener('click', (event) => {
        ...
        if (event.target.classList.contains('product-image')) {
            ...
            Lightbox.open(productId);
            return;
        }
        
        // Otherwise handle product actions
        EventHandler.handleProductAction(event);
    });
